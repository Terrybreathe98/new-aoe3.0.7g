...
- **游戏中**：使用 **W/A/S/D** 持续移动地图视角（可组合为斜向移动）；左键选中单位或建筑，右侧面板显示可用操作（建造、训练、研发、移动、攻击等）；右键空地移动、右键目标攻击/采集。鼠标接触画布边缘不再移动视角。游戏胜利或失败后会自动在 `output/` 下导出 `game_end_结果_时间.txt`，包含结算、资源、单位统计和消息日志。
